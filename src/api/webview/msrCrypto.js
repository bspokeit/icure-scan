  ...
  var msrCrypto = function () {
    ...
    function msrcryptoMath() {
      ...
      function stringToDigits(numberStr, radix) {
        ...
        for (var i = 0; i < numberStr.length; i += 1) {
          ...
          if (isNaN(char)) {
            throw new Error(
              'Failed to convert string to integer in radix ' + radix.toString()
            );
          }
      ...
      function fixedWindowRecode(digits, windowSize, t) {
        ...
        for (var i = 0; i < t; i++) {
          recodedDigits[i] =
            (digits[0] % windowSizeBits) - windowSizeMinus1Bits;
      ...
      function subtract(minuend, subtrahend, difference) {
        ...
        if (minuend.length < subtrahend.length) {
          ...
          if (minuend.length < s) {
            throw new Error(
              'Subtrahend is longer than minuend, not supported.'
            );
          }
        }
        var i,
      ...
      var EllipticCurveOperatorFp = function (curve) {
        ...
        function scalarMultiply(k, point, outputPoint, multiplyBy4) {
          ...
          if (cryptoMath.compareDigits(k, curve.order) >= 0) {
            throw new Error(
              'The scalar k must be in the range 1 <= k < order.'
            );
          }
    ...
    var msrcryptoGcm = function (blockCipher) {
      ...
      function padBlocks(dataBytes) {
        var padLen =
          16 * Math.ceil(mAdditionalBytes.length / 16) -
          mAdditionalBytes.length;
    ...
    if (typeof operations !== 'undefined') {
      msrcryptoKdf.deriveKey = function (p) {
        ...
        var result = msrcryptoKdfInstance.deriveKey(
          p.keyData,
          otherInfo,
          p.derivedKeyType.length
        );
    ...
    if (typeof operations !== 'undefined') {
      ...
      msrcryptoEcdh.exportKey = function (p) {
        ...
        if (p.format === 'jwk') {
          var jsonKeyStringArray = msrcryptoJwk.keyToJwk(
            p.keyHandle,
            p.keyData
          );
          return {
    ...
    var msrcryptoWrapKey = (function () {
      ...
      function unwrapKey(params) {
        ...
        var initialization_vector = b64Tobytes(
